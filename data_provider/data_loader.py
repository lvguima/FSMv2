...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        # M-Stream v4.0: Adjust split to 60/10/30 (12m Train, 2m Val, 6m Test)
        # Original: 12m Train, 4m Val, 4m Test (Total 20m)
...
class Dataset_ETT_minute(Dataset):
    ...
    def __read_data__(self):
        ...
        # M-Stream v4.0: Adjust split to 60/10/30 for minute dataset
        border1s = [0, 12 * 30 * 24 * 4 - self.seq_len, 12 * 30 * 24 * 4 + 2 * 30 * 24 * 4 - self.seq_len]
        border2s = [12 * 30 * 24 * 4, 12 * 30 * 24 * 4 + 2 * 30 * 24 * 4, 12 * 30 * 24 * 4 + 8 * 30 * 24 * 4]
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        # M-Stream v4.0: 调整数据划分 60/10/30 (Train/Val/Test)
...
# 将此代码追加到 FSM/data_provider/data_loader.py 末尾

class Dataset_ETT_minute_Online(Dataset_ETT_minute):
    """
    专门为了对齐 OnlineTSF 框架的 ETTm1 数据划分
    Split: Train(0-4月), Val(4-5月), Test(5-20月)
    """
    def __init__(self, args, root_path, flag='train', size=None,
                 features='S', data_path='ETTm1.csv',
                 target='OT', scale=True, timeenc=0, freq='t', seasonal_patterns=None):
    ...
    def __read_data__(self):
        ...
        # === 核心修改：完全复制 OnlineTSF 的边界定义 ===
        # 1 month = 30 * 24 * 4 = 2880 points
        # Train: 0 ~ 4 months (0 ~ 11520)
        # Val: 4 ~ 5 months (11520 ~ 14400)
        # Test: 5 ~ 20 months (14400 ~ 57600)
        
        border1s = [0, 
                    4 * 30 * 24 * 4 - self.seq_len, 
                    5 * 30 * 24 * 4 - self.seq_len]
...
class Dataset_Custom_Online(Dataset_Custom):
    """
    在线学习版 Custom 数据集 (适用于 Weather 等)
    ...
    Split: Train(20%), Val(5%), Test(75%)
    """
    def __init__(self, args, root_path, flag='train', size=None,
                 features='S', data_path='weather.csv',
                 target='OT', scale=True, timeenc=0, freq='h', seasonal_patterns=None):
        # 直接调用父类初始化，会自动调用子类重写的 __read_data__
        super().__init__(args, root_path, flag, size, features, data_path, target, scale, timeenc, freq, seasonal_patterns)

    def __read_data__(self):
        """
        重写数据加载逻辑，采用在线学习友好的数据划分
        """
        self.scaler = StandardScaler()
        local_fp = os.path.join(self.root_path, self.data_path)
        cfg_name = os.path.splitext(os.path.basename(self.data_path))[0]
        ...
        # 列重排序：['date', ...(other features), target]
        cols = list(df_raw.columns)
        cols.remove(self.target)
        cols.remove('date')
        df_raw = df_raw[['date'] + cols + [self.target]]
        ...
        num_train = int(total_len * 0.20)
        num_vali = int(total_len * 0.05)
